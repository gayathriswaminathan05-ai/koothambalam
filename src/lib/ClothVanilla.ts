...
void main () {
 ...
 vec3 fabric = mix(uBacking, tex.rgb, min(tex.a * 4.0, 1.0));
 float fabricA = tex.a;
 ...
 float shapeA = clamp(0.5 - d, 0.0, 1.0);
 float alpha = fabricA * shapeA;
 outColor = vec4(clamp(lit, 0.0, 1.0) * alpha, alpha);
}`;
...
const PAPER_ALPHA = 0.72;
let grainTile: HTMLCanvasElement | null = null;

/** A tileable 256px grain: per-pixel noise plus soft blotches and a few fibres. */
function getGrainTile() {
  ...
  for (let i = 0; i < 70; i++) {
    ...
    blot.addColorStop(0, dark ? "rgba(60,60,60,0.10)" : "rgba(220,220,220,0.10)");
    blot.addColorStop(1, "rgba(128,128,128,0)");
  ...
  for (let i = 0; i < 90; i++) {
    ...
    g.strokeStyle = Math.random() < 0.6 ? "rgba(70,70,70,0.22)" : "rgba(235,235,235,0.3)";
    g.lineWidth = 0.4 + Math.random() * 0.6;
    g.beginPath();
    g.moveTo(x, y);
    g.quadraticCurveTo(
      x + Math.cos(a) * len * 0.5 + (Math.random() - 0.5) * 4,
      y + Math.sin(a) * len * 0.5 + (Math.random() - 0.5) * 4,
      x + Math.cos(a) * len,
      y + Math.sin(a) * len,
    );
    g.stroke();
...
function paintGrain(ctx: CanvasRenderingContext2D, w: number, h: number, strength: number) {
...
/** Handmade paper in a cool grey (was cream #efe5d2 → #e2d4ba): grey base, grain, and edges that darken like old stock. */
function paintPaper(ctx: CanvasRenderingContext2D, w: number, h: number) {
  const base = ctx.createLinearGradient(0, 0, w * 0.3, h);
  base.addColorStop(0, "#b2b4b6");
  base.addColorStop(1, "#9b9ea1");
  ctx.fillStyle = base;
  ctx.fillRect(0, 0, w, h);
  paintGrain(ctx, w, h, 0.55);
  const edge = ctx.createRadialGradient(w / 2, h / 2, Math.min(w, h) * 0.35, w / 2, h / 2, Math.hypot(w, h) * 0.62);
  edge.addColorStop(0, "rgba(30, 34, 38, 0)");
  edge.addColorStop(1, "rgba(30, 34, 38, 0.3)");
  ctx.fillStyle = edge;
  ctx.fillRect(0, 0, w, h);
  // Slightly translucent stock: the scene behind shows faintly through. Ink and art drawn after stay opaque.
  ctx.save();
  ctx.globalCompositeOperation = "destination-in";
  ctx.fillStyle = `rgba(0, 0, 0, ${PAPER_ALPHA})`;
  ctx.fillRect(0, 0, w, h);
  ctx.restore();
}
...
export function createCloth(
 ...
 function paintFallback() {
  ...
  const cssW = Math.max(1, Math.round(wrapper.clientWidth || content.clientWidth || 1));
  const cssH = Math.max(1, Math.round(wrapper.clientHeight || content.clientHeight || 1));
  if (upload.width !== cssW * dpr || upload.height !== cssH * dpr) {
    upload.width = cssW * dpr;
    upload.height = cssH * dpr;
  }
  ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
  ctx.clearRect(0, 0, cssW, cssH);

  const img = content.querySelector<HTMLImageElement>("img:not(.card-art)");
  const art = content.querySelector<HTMLImageElement>("img.card-art");
  const onPaper = !(img && img.naturalWidth);
  if (!onPaper && img) {
    ...
    paintGrain(ctx, cssW, cssH, 0.16);
    const fade = ctx.createLinearGradient(0, cssH * 0.62, 0, cssH);
    fade.addColorStop(0, "rgba(7,16,24,0)");
    fade.addColorStop(0.45, "rgba(7,16,24,0.38)");
    fade.addColorStop(1, "rgba(7,16,24,0.82)");
    ctx.fillStyle = fade;
    ctx.fillRect(0, cssH * 0.62, cssW, cssH * 0.38);
  } else {
    paintPaper(ctx, cssW, cssH);
  }

  const lab = content.querySelector(".card-lab");
  const padX = Math.max(22, cssW * 0.035);
  const padY = Math.max(28, Math.min(56, cssH * 0.09));
  ...
  const artTop = !!content.closest(".card--art-top");
  if (art && art.naturalWidth && !artTop) {
    ...
    ctx.drawImage(art, box.x + (box.w - aw) / 2, box.y + box.h - ah, aw, ah);
    ctx.restore();
    ...
    if (script?.textContent) {
      ...
      ty += scriptSize;
      ctx.fillText(script.textContent.trim(), padX, ty);
    }
    if (title?.textContent) {
      ctx.fillStyle = ink.title;
      ctx.font = `300 ${bigTitle}px Onest, system-ui, sans-serif`;
      ty += bigTitle + 8;
      ctx.fillText(title.textContent.trim().toUpperCase(), padX, ty);
      ty += 10;
      ctx.fillStyle = "rgba(90, 40, 6, 0.8)";
      ctx.fillRect(padX, ty, 28, 1.5);
      ty += 6;
    }
    if (lede?.textContent) {
      ctx.fillStyle = ink.lede;
      ctx.font = `300 ${small}px Onest, system-ui, sans-serif`;
      const room = Math.max(1, Math.floor((cssH - padY * 0.8 - ty) / lineH));
      for (const l of wrap(lede.textContent, textW, room)) {
        ty += lineH;
  ...
  if (art && art.naturalWidth && artTop) {
    // Wide art above the caption: fit the whole picture in the space left over, never crop it.
    const inset = Math.max(12, Math.min(cssW, cssH) * 0.05);
    const boxH = ledeTop - ledeSize - inset * 1.4;
    const s = Math.min((cssW - inset * 2) / art.naturalWidth, boxH / art.naturalHeight);
    const aw = art.naturalWidth * s;
    const ah = art.naturalHeight * s;
    ctx.save();
    ctx.globalCompositeOperation = "multiply";
    ctx.drawImage(art, (cssW - aw) / 2, inset + (boxH - ah), aw, ah);
    ctx.restore();
  }
  if (ledeLines.length) {
    ctx.fillStyle = ink.lede;
    ctx.font = `300 ${ledeSize}px Onest, system-ui, sans-serif`;
    ledeLines.forEach((l, i) => ctx.fillText(l, padX, ledeTop + i * ledeStep));
  }
  if (title?.textContent) {
    ctx.fillStyle = ink.title;
    ctx.font = `300 ${titleSize}px Onest, system-ui, sans-serif`;
    ctx.fillText(title.textContent.trim().toUpperCase(), padX, y);
  }
  if (script?.textContent) {
    ctx.fillStyle = ink.script;
    ctx.font = `400 ${scriptSize}px "Malayalam Sangam MN", "Noto Sans Malayalam", sans-serif`;
    ctx.textAlign = "right";
    ctx.fillText(script.textContent.trim(), cssW - padX, y);
    ctx.textAlign = "left";
  }
}
 ...
 const srcW = Math.max(content.clientWidth, wrapper.clientWidth, 1);
 contentMaxX = 1;
 if (htmlInCanvas && !content.querySelector("img")) {
 const cssWidth = Math.max(1, Math.round(srcW));
 const cssHeight = Math.max(1, Math.round(Math.max(content.clientHeight, wrapper.clientHeight, 1)));
